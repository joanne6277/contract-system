...
// --- Helper Functions ---
export const getTuFuFieldValue = (obj: any, path: string): any => {
    if (obj === undefined || obj === null) return undefined;

    // Handle virtual/composite columns for sorting
    if (path === 'basicInfo.contractDateRange') return obj.basicInfo?.contractStartDate;
    if (path === 'scopeInfo.thirdPartyPlatform') return obj.scopeInfo?.thirdPartyPlatform_tws;

    let val = path.split('.').reduce((o, i) => o?.[i], obj);
    if (val && typeof val === 'object') {
        if ('selected' in val) return val.selected === 'yes';
        if ('percentage' in val) return val.percentage;
    }
    return val;
};

// --- Field Configuration ---
// Note: TuFu logic relies more on fieldKeyToNameMap for export headers than a structured config for form generation currently.
// We will extract the map and column config.
...
export const fieldKeyToNameMap: { [key: string]: string } = {
    ...
    'otherClauses.shuNiuReserved': '預留欄位',
    ...
    'otherClauses.distributorReserved': '預留欄位',
    ...
    'twBookLogistics.authorizedSalesRegion': '授權銷售地区',
    'twBookLogistics.subDistribution': '轉代理銷售',

    // 台版書 - 窗口資訊
    'twBookContact.publisherRegion': '出版單位地区',
    'twBookContact.contactPersonInfo': '聯絡人資訊',
    'twBookContact.contactPersonTitle': '聯絡人職位',
    'twBookContact.contactPersonEmail': '聯絡人信箱',
    'twBookContact.contactPersonPhone': '聯絡人電話',
    'twBookContact.companyPostalCode': '公司郵遞區號',
    'twBookContact.companyAddress': '公司地址',
    'twBookContact.logisticsPostalCode': '集貨郵遞區號',
...
// Export ordered keys based on the definition order above
export const orderedFieldKeys = Object.keys(fieldKeyToNameMap);
