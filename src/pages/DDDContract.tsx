...
import type { OtherClauses, CheckboxWithTextData } from '@/features/ddd/types';
                                        ...
                                        <div className="space-y-4">
                                            {formData.twBookContact.contacts.map((contact, index) => (
                                                <div key={contact.id} className="p-4 border border-gray-200 rounded-lg relative space-y-4 bg-gray-50/50">
                                                    {formData.twBookContact.contacts.length > 1 && (<Button variant="ghost" size="sm" type="button" onClick={() => removeContact(contact.id)} className="absolute top-2 right-2 text-gray-400 hover:text-red-500 hover:bg-red-50 p-1 h-auto"><Trash2 size={16} /></Button>)}
