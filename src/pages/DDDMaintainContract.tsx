...
const DDDMaintainContract: React.FC = () => {
    const {
        formData,
        message,
        validationErrors,
    ...
    useEffect(() => {
        if (isEditMode) {
            const foundContract = mockDDDContracts.find(c => c.id === id);
            if (foundContract && setFormData) {
                // Deep copy to avoid mutating mock directly if modified in form
                const loadedData = JSON.parse(JSON.stringify(foundContract));
                setFormData(loadedData);
            } else if (isEditMode && !foundContract) {
            ...
            <div ref={mainContentRef} className="max-w-7xl mx-auto">
                ...
                <form onSubmit={handleFormSubmit} className="space-y-8 pb-20">
                    ...
                    <div className="flex justify-end gap-4 mt-8 pb-8">
                        <Button type="button" variant="secondary" onClick={() => window.history.back()}>取消</Button>
                        <Button type="submit"><Save className="w-4 h-4 mr-2" />儲存變更</Button>
                    </div>
                </form>
            </div>
...
export default DDDMaintainContract;
